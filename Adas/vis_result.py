...
import cv2
"""
draw the coco format detectron output
"""
...
def blend_two_images(img1, img2, alpha=0.4):
...
def draw_bbox_and_class(coco,img_name,image,image_id,pred_bbox_json,save_dir,threshold=0.9,thick=1,show_class_and_score=True,font_scale=0.35):
 """
 image: PIL image
 ...
 for i in bbox_file:
  if i['image_id'] == image_id and i['score'] > threshold :
   ...
   if show_class_and_score:
    ...
    txt_str = class_name + '{:0.2f}'.format(i['score'])
...
def main():
 ...
 for j in all_image_Id:
  ...
  draw_bbox_and_class(coco,img_name,img,j,pred_bbox_json,save_dir)
if __name__ == '__main__':
 curr_dir = os.getcwd()
 result_dir = '/data1/shuai/adas/day_night_output/test/adas_train_night/mask_rcnn/segmentations_adas_train_night_results.json'
 dic = json.load(open(result_dir))
 anno_dir = '/data1/shuai/adas/annotations/img_night_train_16000.json'
 img_dir = '/data1/shuai/adas/img_night/'
 pred_bbox_json = '/data1/shuai/adas/day_night_output/test/adas_train_night/mask_rcnn/bbox_adas_train_night_results.json'
 save_dir = '/data1/shuai/adas/day_night_output/test/adas_train_night/mask_rcnn/mask_img/'
 main()
